...
class SingleImage(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Add padding
        # _transform = default_transform_fn(_padding, self._img_size)
        transformed_img_tensor, _ = self._transform(self.img)

        scale = self._img_size / max_size
        scale = torch.tensor([scale])

        padding = torch.tensor(_padding)
        padding = torch.unsqueeze(padding, 0)
        return transformed_img_tensor, scale, padding
